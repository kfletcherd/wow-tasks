...
/**
 * Route for flagging a task as incompleted
 *
 ...
 *  toonId
 */
public final class FlagTaskIncomplete extends SecureRoute {
	...
	throws Exception {
		...
		ps.getDatabaseConnection().execute(
			"UPDATE task_assignment SET completed = false WHERE toon_id = ?::UUID AND task_id = ?::UUID",
			new String[]{ data.get("toonId"), data.get("taskId") }
		);

		ps.res.setCode(200);
		ps.res.setMessage("Task incomplete");
	}
